...
func (r *holePunchResultsXMultiAddressR) GetHolePunchResult() *HolePunchResult {
	if r == nil {
		return nil
	}
...
func (r *holePunchResultsXMultiAddressR) GetMultiAddress() *MultiAddress {
	if r == nil {
		return nil
	}
...
var (
	...
	holePunchResultsXMultiAddressGeneratedColumns      = []string{}
)
...
func (holePunchResultsXMultiAddressL) LoadHolePunchResult(ctx context.Context, e boil.ContextExecutor, singular bool, maybeHolePunchResultsXMultiAddress interface{}, mods queries.Applicator) error {
	...
	if singular {
		var ok bool
		object, ok = maybeHolePunchResultsXMultiAddress.(*HolePunchResultsXMultiAddress)
		if !ok {
			object = new(HolePunchResultsXMultiAddress)
			ok = queries.SetFromEmbeddedStruct(&object, &maybeHolePunchResultsXMultiAddress)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", object, maybeHolePunchResultsXMultiAddress))
			}
		}
	} else {
		s, ok := maybeHolePunchResultsXMultiAddress.(*[]*HolePunchResultsXMultiAddress)
		if ok {
			slice = *s
		} else {
			ok = queries.SetFromEmbeddedStruct(&slice, maybeHolePunchResultsXMultiAddress)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", slice, maybeHolePunchResultsXMultiAddress))
			}
		}
	}
...
func HolePunchResultsXMultiAddresses(mods ...qm.QueryMod) holePunchResultsXMultiAddressQuery {
	...
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"hole_punch_results_x_multi_addresses\".*"})
	}

	return holePunchResultsXMultiAddressQuery{q}
}
