...
func testPeerToManyClients(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := PeerSlice{&a}
	if err = a.L.LoadClients(ctx, tx, false, (*[]*Peer)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testPeerToManyRemoteLatencyMeasurements(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	check, err := a.RemoteLatencyMeasurements().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := PeerSlice{&a}
	if err = a.L.LoadRemoteLatencyMeasurements(ctx, tx, false, (*[]*Peer)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.RemoteLatencyMeasurements); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.RemoteLatencyMeasurements = nil
	if err = a.L.LoadRemoteLatencyMeasurements(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.RemoteLatencyMeasurements); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testPeerToManyAddOpClients(t *testing.T) {
	var err error
...
func testPeerToManyAddOpRemoteLatencyMeasurements(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddRemoteLatencyMeasurements(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.RemoteLatencyMeasurements[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.RemoteLatencyMeasurements[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.RemoteLatencyMeasurements().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
