...
func (r *multiAddressesSetR) GetListenMultiAddressesSetHolePunchResults() HolePunchResultSlice {
	if r == nil {
		return nil
	}
	return r.ListenMultiAddressesSetHolePunchResults
}
...
var (
	...
	multiAddressesSetGeneratedColumns      = []string{"id"}
)
...
func (multiAddressesSetL) LoadListenMultiAddressesSetHolePunchResults(ctx context.Context, e boil.ContextExecutor, singular bool, maybeMultiAddressesSet interface{}, mods queries.Applicator) error {
	...
	if singular {
		var ok bool
		object, ok = maybeMultiAddressesSet.(*MultiAddressesSet)
		if !ok {
			object = new(MultiAddressesSet)
			ok = queries.SetFromEmbeddedStruct(&object, &maybeMultiAddressesSet)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", object, maybeMultiAddressesSet))
			}
		}
	} else {
		s, ok := maybeMultiAddressesSet.(*[]*MultiAddressesSet)
		if ok {
			slice = *s
		} else {
			ok = queries.SetFromEmbeddedStruct(&slice, maybeMultiAddressesSet)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", slice, maybeMultiAddressesSet))
			}
		}
	}
...
func MultiAddressesSets(mods ...qm.QueryMod) multiAddressesSetQuery {
	...
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"multi_addresses_sets\".*"})
	}

	return multiAddressesSetQuery{q}
}
...
func (o *MultiAddressesSet) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		wl = strmangle.SetComplement(wl, multiAddressesSetGeneratedColumns)
...
func (o *MultiAddressesSet) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		insert = strmangle.SetComplement(insert, multiAddressesSetGeneratedColumns)
		update = strmangle.SetComplement(update, multiAddressesSetGeneratedColumns)
